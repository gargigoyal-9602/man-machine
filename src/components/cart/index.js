...
function CartCheckoutform(props) {
  ...
  const [hyperPayId, setHyperPayId] = useState();
  ...
  const [frameSizeInterval, setFrameSizeInterval] = useState();
  ...
  useEffect(() => {
    if (values !== undefined) {
      props.setShouldShowDeliveryCharge(true);
                    ...
                    <div class="yt-chekout-radio d-flex flex-wrap my-3 justify-content-between">
                      <div className="yt-cart-radio-slct-wrap d-flex flex-wrap align-items-center">
                        <FormGroup className="mr-2">
                          <span class="checkout-form-label">
                            {confirmordercartlang.get("noPaymentRequired", "No Payment Required")}
                          </span>
...
function CartPageCheckoutContent(props) {
  return (
    <div class="cart-checkout-wrap">
      <Row>
        <Col md={12}>
          <CartCheckoutform data={props.data} setShouldShowDeliveryCharge={props.setShouldShowDeliveryCharge} />
        </Col>
      </Row>
    </div>
  );
...
function CartProductListData() {
  ...
  const [shouldShowDeliveryCharge, setShouldShowDeliveryCharge] = useState(false);
  const cartState = CartState.get();
  const history = useHistory();
  ...
  function getProducts() {
    let products = [];
    let items = cartState.cart?.order_items;
    debugger
          ...
          <Row className="yt-cm-row">
            <Col md={7} className="yt-cm-lt-col">
              {ShowLogin ? (
                ...
                <Fragment>
                  {ShowCheckout ? (
                    <CartPageCheckoutContent data={data} setShouldShowDeliveryCharge={setShouldShowDeliveryCharge} />
                  ) : (
                    <Fragment>
                      {getProducts()}
                    </Fragment>
            ...
            <Col md={5} className="yt-cm-rt-col">
              {(ShowCart || ShowCheckout) && <CartAmount cart={cartState.cart} data={data} shouldShowDeliveryCharge={shouldShowDeliveryCharge} />}
            </Col>
          </Row>
      ...
      cart.order_items.forEach((item, idx) => {
        ...
        items.push(
          <tr key={idx}>
            ...
            <td>
              <span class="cart-product-amount-qty">
                x{item.quantity}
              </span>
            </td>
            <td>
              <span class="cart-product-amount-price">{data.currency} {numberWithCommas(item.total_price)}</span>
            </td>
          </tr>
        )
      });
      ...
      <Table className="mb-0 cart-prodict-amount " borderless>
        <thead>
          <tr>
            ...
            <th>
              {confirnordercartlang.get("qty", "Qty")}</th>
            <th>
        ...
        <tbody>
          <tr>
            ...
            <td style={{ paddingRight: 0 }}>
              <span class="cart-product-amount-price cart-sub-total">{data.currency} {numberWithCommas(data.subTotal)}</span>
            </td>
          </tr>
        </tbody>
      </Table>
      ...
      <Table className="mb-0 cart-prodict-total-amount " borderless>
        <tbody>
          <tr>
            ...
            <td>
              <span class="cart-product-amount-price">+ {data.currency} {numberWithCommas(data.tax)}</span>
            </td>
          </tr>
         {props.shouldShowDeliveryCharge && <tr>
            <td>
              <span class="cart-product-amount-ttl">
                {confirnordercartlang.get("deliveryCharges", "Delivery Charges")}</span>
            </td>
            <td>
              <span class="cart-product-amount-price">+ {data.currency} {numberWithCommas(data.deliveryCharges)}</span>
            </td>
          </tr>}
        </tbody>
      </Table>
            ...
            <Table className="mt-2 mb-0 cart-prodict-total-amount " borderless>
              <tbody>
                <tr>
                  ...
                  <td>
                    <span class="cart-product-amount-price">- {data.currency} {numberWithCommas(data.discount)}</span>
                  </td>
                </tr>
              </tbody>
            </Table>
      ...
      <Table className="mb-0 cart-prodict-sub-total-amount " borderless>
        <tbody>
          <tr>
            ...
            <td>
              <span class="cart-product-amount-price cart-sub-total">{data.currency} {numberWithCommas(data.totalAmount)}</span>
            </td>
          </tr>
        </tbody>
      </Table>
...
function CartProduct({ product, ...props }) {
  ...
  function getColorAndSize() {
    let properties = product.product_variant?.product_variant_properties;
  ...
  function moveToWishlist(product, onSuccess) {
    ...
    service(serviceType)
      .then(({ response, error }) => {
        if (error) {
          ...
          window.notify([{ message: "Product moved to wishlist successfully", type: "success" }]);
          onSuccess();
        }
    ...
    <div class="cart-produt-list-wrap radius-10 bg-white cart-pg-mb-30">
      <div className="d-flex flex-wrap cart-pg-product-list-row justify-content-between">
        ...
        <div className="cart-pg-list-prdt-info d-flex justify-content-between">
          <div class="cart-prodict-info">
            <div className="cart-product-head-container">
              <h2 class="cart-product-title mt-0 w-100 w3-ripple" style={{ cursor: "default" }} onClick={() => history.push(`/shop/${product?.product?.id}`)}>
                {product.product_name}
              </h2>
              <div class="cart-action-wrap text-right w-100">
                {!product?.product?.is_wishlisted &&
                  <Fragment>
                    <div class="cart-product-wishlist p-2 w3-ripple" onClick={() => moveToWishlist(product?.product, () => commands.cart.removeItem({ productId: product?.product?.id, variantId: product?.product_variant?.id }))}>
                      Move to Wishlist
              </div>
                    <span className="cart-action-separator">
                      |
              </span>
                  </Fragment>
                }
            ...
            <div className="yt-product-code text-left pt-2">
            ...
            <span class="cart-product-price"> {product.currency} {numberWithCommas(product.price)}</span>
          </div>
          ...
          <div class="cart-prodict-info">
            ...
            <div className="cart-product-head-container">
              ...
              <div class="cart-action-wrap text-right w-100">
                {!product?.product?.is_wishlisted &&
                  <Fragment>
                    <div class="cart-product-wishlist p-2 w3-ripple" onClick={() => moveToWishlist(product?.product, () => commands.cart.removeItem({ productId: product?.product?.id, orderItemId: product.id }))}>
                      Move to Wishlist
                  </div>
                    <span className="cart-action-separator">
                      |
