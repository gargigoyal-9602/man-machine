...
function ConnectAccounts() {
  ...
  function statusChangeCallback(response) {
    if (response.status === 'connected') {
      ...
      commands.user.connectSocialAccount(body, () =>
        commands.user.getSocialList()
      );
    } else if (response.status === 'not_authorized') {
      // The person is logged into Facebook, but not your app.
      window.notify([
        { message: 'Error while connecting with Facebook', style: 'danger' },
      ]);
    }
  }
  ...
  let googleData = Array.isArray(cacheState.social_connects)
    ? cacheState.social_connects.filter(
        (item, idx) => item.provider === 'google'
      )
    : [];
  const isGoogleConnected = googleData.length > 0;
  googleData = isGoogleConnected ? googleData[0] : {};

  let facebookData = Array.isArray(cacheState.social_connects)
    ? cacheState.social_connects.filter(
        (item, idx) => item.provider === 'facebook'
      )
    : [];
        ...
        <div className="profile-pg-inner-wrapper">
          <div class="pp-ca-main-wrap">
            ...
            <ul class="m-0 p-0 pp-ca-list-none pp-ca-listing">
              <li
                className={classnames({ active: isFacebookConnected === true })}
              >
                <div class="d-flex justify-content-between align-items-center">
                  <div
                    style={isFacebookConnected ? {} : { cursor: 'pointer' }}
                    class={
                      'pp-ca-type d-flex align-items-center' +
                      (isFacebookConnected ? '' : 'w3-ripple')
                    }
                    onClick={isFacebookConnected ? undefined : connectFacebook}
                  >
                    <img
                      ...
                      {isFacebookConnected === true ? (
                        <Fragment>
                          <span class="pp-ca-tag-name  mb-2">
                            {connectAccountslang.get(
                              'connectedAs',
                              'Connected as'
                            )}
                          </span>
                          <h2 class="pp-ca-user-name mt-0 mb-0 w-100">
                            {facebookData.display_name}
                          </h2>
                        </Fragment>
                      ) : (
                      ...
                      <img
                        ...
                        onClick={() => {
                          setDisconnectFunction(() => {
                            return () =>
                              commands.user.removeSocialAccount(
                                facebookData.id,
                                () => commands.user.getSocialList()
                              );
                          });
              ...
              >
                <div class="d-flex justify-content-between align-items-center">
                  <div
                    style={isGoogleConnected ? {} : { cursor: 'pointer' }}
                    class={
                      'pp-ca-type d-flex align-items-center' +
                      (isGoogleConnected ? '' : 'w3-ripple')
                    }
                    onClick={isGoogleConnected ? undefined : connectGoogle}
                  >
                    <img
