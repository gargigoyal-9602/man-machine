...
function ItemWithRouter(props) {
  ...
  return (
    <ListGroupItem
      ...
      onClick={() => {
        if (props.query) {
          commands.products.setRecentSearches({
            query: props.query,
        ...
        switch (props.itemClassName) {
          case 'Category':
            history.push(
              '/shop?page=1&per_page=15&category_id[]=' + props.productId
            );
            break;
          case 'SubCategory':
            history.push(
              `/shop?page=1&per_page=15&category_id[]=${
                props.categoryId
              }&sub_category_id[]=${props.productId}`
            );
            break;
          case 'Product':
...
class ExitSearch extends Component {
  constructor(props) {
    super(props);
  }
        ...
        <div className="d-flex align-items-center justify-content-between mb-4">
          <p className="recent-search-tag-name m-0">
            {this.props.isQuickResults
              ? lang.get('quickResults', 'Quick Results')
              : lang.get('recentSearches', 'Recent Searches')}
          </p>
          <img
            ...
            return (
              index < 10 && (
                <ItemWithRouter
                  ...
                  query={this.props.isQuickResults ? item.name : undefined}
                  productId={item.class_id}
                  categoryId={item.category_id}
                  itemClassName={item.class_name}
                >
                  {item.name}
                </ItemWithRouter>
              )
