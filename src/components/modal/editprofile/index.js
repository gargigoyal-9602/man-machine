...
function EditProfileModal(props) {
  ...
  const [isProfilePics, setisProfilePics] = useState(
  ...
  function openImageUpload() {
    let fileTag = document.getElementById('user_profile_pics');
  ...
  const saveProfile = async ({ name, email, phoneno }) => {
    ...
    if (removePicture) {
      ...
      userDetails.profile_picture = submitImage
        ? croppedImage || isProfilePics
        : undefined;
    }

    commands.user.updateProfile(
      userDetails,
      () => {
  ...
  return showCropper ? (
    ...
    <div>
      ...
      >
        ...
        >
          <span>{profileeditlang.get('editProfile', 'Edit Profile')}</span>
        </ModalHeader>
        <ModalBody className="yt-edit-prfl-body">
          <Formik
