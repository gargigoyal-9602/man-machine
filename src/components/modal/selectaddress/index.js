...
function SelectAddressModal(props) {
  ...
  return ShowADModal ? (
    <AllModal
      ...
      setAddress={() => null}
      isOpen={ShowADModal}
      toggle={() => setShowADModal(!ShowADModal)}
    />
  ) : (
    <div>
      <Modal
      ...
      >
        ...
        >
          <span>{selectlang.get('selectAddress', 'Select Address')}</span>
        </ModalHeader>
            ...
            <div class="profile-pg-inner-wrapper">
              <div class="profile-pg-sa-address-main-wrap">
                <ul class="pp-sa-list-none p-0 m-0 pp-sa-all-addres-list">
                  {Array.isArray(userState?.address) &&
                  userState?.address.length > 0 ? (
                    userState?.address.map((item, idx) => (
                      <li
                        className={classnames({
                          active: activeTab === idx.toString(),
                        })}
                        onClick={() => {
                ...
                <Button
                  color="link d-none yt-mb-add-new-address-nbtn"
                ...
                onClick={
                  /*openAddAddressModal*/ () =>
                    userState?.address?.length > 0
                      ? props.onContinue({
                          type: props.type,
                          address: userState?.address[activeTab],
