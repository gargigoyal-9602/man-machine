...
import { BsStarFill, BsStarHalf, BsChevronDown } from "react-icons/bs";
  ...
  return (
    <div className="right-inner-content-wrap">
      ...
      <AllModal
        modalName="SizeChart"
        isOpen={sizeChart}
        toggle={openSizeChartModal}
        images={
          Array.isArray(props.product?.size_charts) &&
          props.product?.size_charts.length > 0 &&
          props.product.size_charts[0]?.image
            ? props.product.size_charts
            : []
        }
      />
      ...
      <div className="sp-inner-wrap bg-white radius-10">
        ...
        <div className="yt-product-code text-left">
          Product Code: {props.product.id}
        </div>
        <div
          className="text-left mt-2"
          style={{ opacity: props.product.total_ratings > 0 ? 1 : 0 }}
        >
          <span>
            {parseFloat(props.product?.average_rating)?.toFixed(1) || 0}{" "}
            <IoIosStar className="rating-star-icon" />{" "}
            <span className="yt-product-rating ">
              | {props.product.total_ratings}
            </span>
          </span>
        </div>
        ...
        <div className="sp-price-wrap d-flex flex-nowrap align-items-center justify-content-between">
          <div className="sp-price-left-content">
              ...
              <ul className="list-style-none p-0 my-2">
                ...
                <li className="d-inline-block">
                  <p
                    ...
                    style={{ color: "#8b8f95" }}
                  >
                    <strike>₹INR {props.product?.price}</strike>
                  </p>
                </li>
              </ul>
            ) : (
            ...
            {props.product.stock_qty >= 1 &&
            props.product.current_availability === "in_stock" ? (
              <Fragment>
                <div className="d-flex align-items-center">
                  ...
                  <p className="m-0 sp-quantity-tag-name">
                    {lang.get("inStock", "In stock online")}
                  </p>
                </div>
                {Array.isArray(props.product?.size_charts) &&
                  props.product?.size_charts.length > 0 && (
                    <div
                      className="yt-prdt-size-chart in stock"
                      onClick={openSizeChartModal}
                    >
                      <p className="m-0 mt-2 sp-size-chart-tag-name">
                        {lang.get("sizeChart", "Size Chart")}
                      </p>
                    </div>
                  )}
              </Fragment>
                ...
                <div className="d-flex align-items-center">
                  {!props.product.anyOtherVariantInStock && (
                    <Fragment>
                      ...
                      <p className="m-0 mr-2 sp-out-quantity-tag-name">
                        {lang.get("soldOut", "Sold Out")}
                      </p>
                    </Fragment>
                  )}
                </div>
            ...
            {props.product.product_attributes?.size?.length > 0 && (
              <div>
                <div className="variant-dropdown-title">Size</div>
                <Dropdown
                  isOpen={openSizeDropdown}
                  toggle={() => setOpenSizeDropdown((prev) => !prev)}
                >
                  <DropdownToggle className="variant-dropdown">
                    {
                      props.product.product_attributes?.size.find(
                        (item, idx) => item.active
                      ).name
                    }{" "}
                    <BsChevronDown className="variant-dropdown-icon" />
                  </DropdownToggle>
                  <DropdownMenu>
                    {props.product.product_attributes?.size.map((item, idx) => (
                      <DropdownItem
                        onClick={() =>
                          props.product.setActiveAttribute("size", item.name)
                        }
                      >
                        {item.name}
                      </DropdownItem>
              ...
              <div className="sp-price-right-content">
                <div className="d-flex align-items-center justify-content-end">
                  <p className="m-0 sp-quantity-tag-name pr-2">
                    {lang.get("quantity", "Quantity")}
                  </p>
                  <div className="sp-quantity-box">
                    <CartQuantityField
                      maxQuantity={props.product.stock_qty}
                      CartQuantityVal={props.product.quantity}
                      setCartQuantityVal={props.product.setQuantity}
                    />
                  </div>
                </div>
              </div>
        ...
        {props.product.product_attributes?.color?.length > 0 && (
          <div className="sp-price-wrap  sp-border-top-bottom yt-mb-space-border pb-0">
            <p className="m-0 sp-small-tag-name">
              {lang.get("color", "COLOR")}
            </p>
              ...
              {props.product.product_attributes?.color.map((item, idx) => (
                <li className={idx === 0 ? "" : "mx-2"}>
                  <div
                    ...
                    onClick={() =>
                      props.product.setActiveAttribute("color", item.name)
                    }
                  />
                </li>
              ))}
        ...
        {Object.keys(props.product.product_attributes || {}).map(
          (key) =>
            !["color", "size"].includes(lowerCase(key)) &&
            props.product.product_attributes[key]?.length > 0 && (
              <div className="sp-size-wrap pb-0 sp-other-wrap">
                <p className="m-0 sp-small-tag-name">{upperCase(key)}</p>
                <ul
                  className="mb-0 p-0 mt-2 list-style-none d-flex flex-wrap align-items-center justify-content-start"
                  id="sp-size-data"
                >
                  {props.product.product_attributes[key].map((item, idx) => (
                    ...
                    >
                      <div
                        ...
                        data-item="XS"
                        onClick={() =>
                          props.product.setActiveAttribute(key, item.name)
                        }
                      >
                        {item.name}
                      </div>
                  ...
                  <button
                    ...
                    onClick={() =>
                      onBuyNow({
                        ...
                        variantId: props.product.variant_id || '',
            ...
            <li className="mx-2">
              ...
              */}
              {props.product.anyOtherVariantAvailable ? (
                <p className="product-stock-message mb-0">
                  {!props.product.variantAvailable
                    ? lang.get(
                        "combinationNotAvailable",
                        "This combination is not available. Please select from the available variants."
                      )
                    : lang.get(
                        "itemOutOfStock",
                        "The Item is currently out of stock."
                      )}{" "}
                  {props.product.is_notify_product &&
                    props.product.variantAvailable &&
                    lang.get(
                      "youWillGetNotified",
      ...
      <div className="sp-inner-wrap bg-white radius-10 ">
        ...
        <div className="sp-price-wrap d-flex flex-nowrap align-items-center justify-content-between">
          <div className="sp-price-left-content">
            ...
            <ul className="list-style-none p-0 my-2 d-flex align-items-center">
              <li>
                <p className="sp-price-tag-value m-0">₹INR 120</p>
              </li>
              <li>
                <div className="d-flex align-items-center pl-3">
                  ...
                  <div
                    className="yt-prdt-size-chart yt-out"
                    onClick={openSizeChartModal}
                  >
      ...
      <div className="d-flex align-items-center justify-content-center justify-content-md-end ">
        <ul className="p-0 list-style-none d-flex align-items-center sp-product-add-action-wrap">
          ...
          <li className="mx-2">
            <button
              type="button"
              className="btn btn-light sp-addto-cart-btn notify py-3"
...
function ProductDetails(props) {
  ...
  */
      ...
      .then(({ response, error }) => {
        if (error) {
          ...
          if (
            ...
            typeof response.data.data.product === "object"
          ) {
            setProduct(omitBy(response.data.data.product, isNil));
            setSimilarProducts(response?.data?.data?.similar_products);
            let reviewData = response?.data?.data?.logged_in_user_reviews || [];
            let orderItemId = response?.data?.data?.order_item_id;
            ...
            if (orderItemId && orderId) {
              let canReview =
                response?.data?.data?.can_review &&
                !(
                  Array.isArray(reviewData) &&
                  reviewData.find((item) => item.order_id === orderId)
                );
    ...
    if (productReviews?.meta?.pagination?.next_page || !productReviews) {
      commands.products
        .getReviews({
          ...
          perPage: productReviews?.meta?.pagination?.total_count * 2 || 5,
        })
        .then((val) => {
          if (val) {
  ...
  useEffect(() => {
    if (product === undefined) {
      ...
      try {
        ...
        window.notify([
          { message: "Error in Processing Data", type: "danger" },
        ]);
      }
    }
  }, [product]);
  ...
  function getCurrentProduct() {
    ...
    let clone = cloneDeep(product);

    let variantIndex = undefined;

    if (
      Object.keys(clone.product_attributes || {}).find(
        (item) => !(item in activeAttributes)
      )
    ) {
      clone.product_variants.forEach((vitem) => {
        if (vitem.is_master) {
      ...
      clone.product_variants.forEach((vitem, vidx) => {
        ...
        vitem.product_variant_properties.forEach((item, idx) => {
          ...
          if (
            !(
              item.variant_name in activeAttributes &&
              item.property_name === activeAttributes[item.variant_name]
            )
          ) {
            found = false;
          }
    ...
    variantIndex === undefined &&
      (clone?.stock_qty || 0) - (clone?.block_qty || 0);
    ...
    if (variantIndex !== undefined) {
      ...
      Array.isArray(clone.product_variants) &&
      clone.product_variants.length > 0
    ) {
    ...
    if (clone.product_attributes) {
      Object.keys(clone.product_attributes).forEach((key) => {
        clone.product_attributes[key].forEach(
          (item) =>
            (item.active = item.name === activeAttributes[key] ? true : false)
        );
      });
    }
    ...
    if (
      product !== undefined &&
      (currentImage === "null" ||
        clone.images.filter((item, idx) => item.image === currentImage).length <
          1)
    ) {
  ...
  return (
    <div>
      ...
      {product ? (
        <Container>
          <AllModal
            ...
            onSuccess={() => {
              props.history.replace("/shop/" + props.match?.params?.productId);
              getProductDetails();
            }}
          />
                  ...
                  <div className="sp-inner-content-wrap bg-white radius-10">
                    <div
                      className={
                        ProductDesc
                          ? "sp-description-wrap"
                          : "sp-description-wrap active-desc"
                      }
                    >
                      <h2 className="sp-description-title mt-0">
                        {lang.get("description", "Description")}
                      </h2>
                      ...
                      <Button
                        color="link yt-read-more-des p-0 d-none"
                        onClick={ProductDscClick}
                      >
                  ...
                  <div className="sp-inner-content-wrap bg-white radius-10">
                    <div className="sp-rating-wrap pb-4">
                      ...
                      <Row className="align-items-center yt-rating-row">
                        <Col
                          ...
                          lg={12}
                          xl={8}
                          className="yt-rating-col"
                        >
                          <div
                            className="d-flex flex-wrap align-items-center justify-content-between sp-border-md-right yt-rating-inner-row"
                            style={canReview ? {} : { border: "unset" }}
                          >
                            <div className="sp-rating-inner-wrap">
                              ...
                              {lang.direction === "rtl" ? (
                                <span className="yt-based-on">
                                  {lang.get("ratings", "Ratings")}{" "}
                                  {product.total_review}{" "}
                                  {lang.get("basedOn", "Based on")}
                                </span>
                              ) : (
                                <span className="yt-based-on">
                                  {lang.get("basedOn", "Based on")}{" "}
                        ...
                        >
                          ...
                          >
                            <h2 className="sp-description-title mt-0 m-0 text-center">
                              {lang.get("productRatingSmall", "Rate Product")}
                            </h2>
                            <ul className="list-style-none p-0 my-3 d-flex align-items-center justify-content-center">
                              {times(5, () => (
                                <li className="mr-1">
                                  <img
                                    src={require("./images/star.png")}
                            ...
                            <button
                              type="button"
                              className="btn btn-light sp-give-review-btn py-3 w-100"
                              data-v-b6eeb6e1=""
                    ...
                    <div className="sp-rating-wrap pt-4 border-top-grey">
                      {productReviews?.review.map((item, idx) => (
                        <div
                          key="idx"
                          className="d-flex align-aitems-center justify-content-between py-3 yt-sp-user-rate-box"
                        >
                              ...
                              >
                                <img
                                  src={
                                    ...
                                    require("./images/user-alt-512.png")
                                  }
                                  alt="user"
                                  className="img-fluid"
                          ...
                          <div className="sp-user-rated-meta text-right">
                            ...
                            <div className="sp-rating-date text-nowrap">
                              {item.review_date}
                            </div>
                          </div>
                        </div>
                      ...
                      {productReviews?.meta?.pagination?.next_page && (
                        <div>
                          <Button
                            color="link yt-read-more-des p-0 w3-right"
      ...
      <ProductCard
        products={[
          ...similarProducts /*,...cacheState.homepage?.recommended_products*/,
        ]}
        name={lang.get("similarProduct", "Similar Product")}
        onViewMore={() =>
          props.history.push(
            `/shop?page=1&per_page=15&category_id[]=${
              product?.sub_category?.category_id
