...
export default async function updateAddress(address, addressId, onSuccess, onFailure) {
    try {
        ...
        if (typeof user === "object") {
            serviceType = serviceTypes.updateAddress(user?.id, addressId);
        } else {
            throw new Error("Login data error, try logout and login again.");
        }

        if (serviceType) {
            serviceType.body.address = address;
        }
